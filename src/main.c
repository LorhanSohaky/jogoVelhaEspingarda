...
void print_jogadas( int *tabuleiro ) {
    printf( "<input type=\"hidden\" name=\"teste\" value=\"" );
    int i, j;
    for( i = 0; i < lin; i++ ) {
        for( j = 0; j < col; j++ ) {
            printf( "%d %d %d;", tabuleiro[lin * i + j], i, j );
        }
    }
...
void print_jogo( int *tabuleiro ) {
    ...
    print_jogadas( tabuleiro );
}

void print_form( int *tabuleiro ) {
    printf( "<form method=\"GET\" action=\"main\">" );
    print_jogadas( tabuleiro );
